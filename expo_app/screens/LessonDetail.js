// expo_app/screens/LessonDetail.js (CLEANED)
...
    Alert.alert('Marked done', `Lesson '${lesson.title}' completed for child ${childId}`); // Added context to Alert
  }

  if (loading) return <ActivityIndicator style={{flex:1}} size="large" />; // Added size attribute

  return (
    <View style={styles.container}>
      <Text style={styles.title}>{lesson.title}</Text>
      {lesson.type === 'video' && fileUrl ? (
        <Video source={{ uri: fileUrl }} style={styles.mediaPlayer} useNativeControls resizeMode="contain" />
      ) : lesson.type === 'pdf' && fileUrl ? (
        <WebView source={{ uri: `https://docs.google.com/gview?embedded=true&url=${encodeURIComponent(fileUrl)}` }} style={styles.webView} />
      ) : (
